...
3. Run `npm run prisma:migrate` to generate + apply migration.
4. Run `npm run prisma:generate` to update generated types (the migrate dev command usually does this automatically, but do it explicitly if types are stale).
